...
from src.extractor import ExtractorRule, ExtractorDL, ExtractorCompound, ExtractorLLM
...
def generate_chains_key_info_dl(use_log:bool=False, ext_type:str='dl'):
    """
        使用DL model生成key info
        See: https://huggingface.co/sentence-transformers
    """

    def extract_fields_dl(data: T.List[T.List[T.Dict]], chain: str, extractor:ExtractorDL):
        ext = extractor
        trx_key_info_list = []
        for one_trx in data:
            if not len(one_trx[0]): 
                # empty results from previous step
                continue
            try:
                extracted_info = ext.go(one_trx[0], 
                    one_trx[1:] if len(one_trx) > 1 else [], chain,C.info_ext_config[C.current_bridge])
            except Exception as e:
                logging.exception(e)
                logging.error("transaction deal fail. Hash:%s" % (one_trx[0]['hash']))
                continue
            extracted_info.update({"hash":one_trx[0]['hash'], 'chain':chain})
            if len(extracted_info):
                trx_key_info_list.append(extracted_info)
        return trx_key_info_list

    print('---extract begin---')
    from sentence_transformers import SentenceTransformer

    model = SentenceTransformer(C.paths().model.bge, device='cuda').cuda()
    if ext_type == 'dl':
        extractor = ExtractorDL(model)
    else:
        extractor = ExtractorCompound(model)
    chains_key_info = {}
    dec_data = json.load(open(C.paths().decoded_data))
    for chain in dec_data:
...
def generate_chains_key_info_llm(LLM_api_conf_name:str='', no_check_rpc:bool=False):
    ...
    for chain in C.chains_to_pair:
        ...
        valid_trx = 0
        for i, one_trx in enumerate(dec_data[chain]):
            if not len(one_trx[0]): continue
            a, _info  = ext.go_for_voting(one_trx[0], one_trx[1:] if len(one_trx) > 1 else [], chain)
            if a > 0 :
                valid_trx += 1
                if valid_trx % 100 == 0: ext._save_LLM_record()
            else:
                voting_statis[_info] += 1

        print(f"{chain}, Valid:{valid_trx}")
        if len(voting_statis) > 0: print(f"Voting Statis: {TL.defaultdict_to_dict(voting_statis)}")
...
def main(args):
    ...
    if args.phase == 'all' or args.phase == 'extract':
        if args.ext_type == 'dl':
            generate_chains_key_info_dl(True, 'dl')
        elif args.ext_type == 'comp':
            generate_chains_key_info_dl(True, 'comp')
        elif args.ext_type == 'rule':
            generate_chains_key_info_rule(True)
    ...
    # if args.phase == 'all' or args.phase == 'gt':
    #     from src.gt_generator import generate_gt, generate_paired_tokens_from_gt
    #     if C.current_bridge.startswith('Multichain'):
    #         generate_gt()
    #         generate_paired_tokens_from_gt()
    #     elif C.current_bridge.startswith('Optimism'):
    #         generate_paired_tokens_from_gt()
