...
def generate_chains_key_info_dl(no_check_rpc:bool=False, heuristic:bool=False):
    """
    ...
    extractor = ExtractorSim(model, '', not no_check_rpc, heuristic)
...
def main(args):
    ...
    if args.phase == 'all' or args.phase == 'extract':
        if args.ext_type == 'dl':
            generate_chains_key_info_dl(args.no_check_rpc, args.heu)
...
if __name__ == '__main__':
    ...
    argp.add_argument('--heu', action="store_true" ,default=False, required=False, help='if set to true, combine the DL model with heuristic filtering rules. Only valid when --ext_dl is set. NOTE: if you specify this variable, then the chains_to_pair in config file will be override.')
