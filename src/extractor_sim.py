...
class ExtractorSim(ExtractorLLM):
    ...
    def __init__(self, sim_model, model_type:str='gpt-4o-mini', check_rpc:bool=True, heuristic:bool=False):
        self.encoder:SentenceTransformer = sim_model
        self.model:SentenceTransformer = self.encoder
        self.pre_encoded_queries = []
        self.heuristic_types = {
            C.PRESET_KEYS_TIMESTAMP: (int, str),  # Some datasource that construc timestamp with string, while others provide int. 
            C.PRESET_KEYS_AMOUNT: int,
            C.PRESET_KEYS_TO:(str, bytes), 
            C.PRESET_KEYS_CHAIN: int,
            C.PRESET_KEYS_TOKEN: (str, bytes)
        }
        self.filter_with_heu = heuristic # if True, filter the fields first to employ heuristic based filtering method
    ...
    def filter_keys_with_types(self, trx, logs, all_fields, type_wanted, key_name:str=''):
        """
            Given `type_wanted`, return the corresponding keys that meet the requiments.
        """
        ret = []
        for one_key in all_fields:
            dotted_fields = self._convert_fields_to_dot(one_key)
            v = TL.extract_values_by_given_fields(trx, logs, {key_name: [dotted_fields]}, True)
            try:
                v = v[key_name]
            except: continue
            if isinstance(v, type_wanted): ret.append(one_key)
        return ret
    ...
    def _encode_and_extract_from_given_keys_queries(
        ...
        for i, p in enumerate(query_values):
            if self.filter_with_heu: 
                all_keys_filtered = self.filter_keys_with_types(trx, logs, all_keys_old, self.heuristic_types[C.PRESET_KEYS[i]], C.PRESET_KEYS[i] )
                all_keys = list(map(lambda x: _cut_varnames(x), all_keys_filtered))
                all_keys_embedding = self.encoder.encode(all_keys, show_progress_bar=False)
            else: 
                all_keys_filtered = copy.deepcopy(all_keys_old)
            all_keys_sim_scores = {all_keys[j] : float(util.cos_sim(p, x).squeeze()) for (j, x) in enumerate(all_keys_embedding)}
            ...
            for _key_in_top5 in top5_keys:
                ...
                _key_old = all_keys_filtered[index]
